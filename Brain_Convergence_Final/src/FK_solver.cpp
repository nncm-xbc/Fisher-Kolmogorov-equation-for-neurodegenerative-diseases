...
FK_solver::assemble_system()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    ...
    FullMatrix<double> D_matrix(dim, dim);
    D.matrix_value(fe_values.quadrature_point(q), D_matrix);
    ...
    for (unsigned int i = 0; i < dofs_per_cell; ++i) {
        for (unsigned int j = 0; j < dofs_per_cell; ++j) {
            ...
            Tensor<1, dim> D_grad_phi_j;
            
            //compute the matrix vectol product
            for (unsigned int d = 0; d < dim; ++d) {
              //D_grad_phi_j[d] = 0.0;
              for (unsigned int e = 0; e < dim; ++e) {
                D_grad_phi_j[d] += D_matrix(d, e) * grad_phi_j[e];
              }
            }

            cell_matrix(i, j) += scalar_product(D_grad_phi_j, grad_phi_i) *
                                 fe_values.JxW(q);
        }
        ...
        Tensor<1, dim> D_grad_loc_q;

        for (unsigned int d = 0; d < dim; ++d){
            //D_grad_loc_q[d] = 0.0;
            for (unsigned int e = 0; e < dim; ++e){
               D_grad_loc_q[d] += D_matrix(d, e) * solution_gradient_loc[q][e];
            }
         }

        cell_residual(i) -= scalar_product(D_grad_loc_q, fe_values.shape_grad(i, q)) 
                            * fe_values.JxW(q);
...
void
FK_solver::output(const unsigned int &time_step) const
{
